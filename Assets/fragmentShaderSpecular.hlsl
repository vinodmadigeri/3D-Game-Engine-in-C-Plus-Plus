...
uniform float g_specular_shininess = 1.0;
...
void main(in const float3 i_position_world : POSITION1, in const float3 i_normal_world : NORMAL, in const float2 i_uv : TEXCOORD0, in const float3 i_color_perVertex : COLOR0,
	out float4 o_color : COLOR0 )
{
	...
	float3 lighting_specular;
	{
		 ...
		 lighting_specular = g_lighting * specularAmount;
	}
	
	// The final lit color is the albedo modified by the diffuse lighting, ambient lighting and specular lighting
	float3 color_lit = color_albedo * ( lighting_diffuse + g_lighting_ambient + lighting_specular);
