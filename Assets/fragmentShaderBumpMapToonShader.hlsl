...
void main( in const float3 i_normal_world : NORMAL, in float3 i_tangent_world : TANGENT, in float3 i_bitangent_world : BINORMAL, in const float2 i_uv : TEXCOORD0, in const float3 i_color_perVertex : COLOR0,
	out float4 o_color : COLOR0 )
{
	...
	if (diffuseAmount > 0.95)
        color_lit = float3(1.0,1.0,1) * color_lit;
	else if (diffuseAmount > 0.5)
        color_lit = float3(0.7,0.7,0.7) * color_lit;
	else if (diffuseAmount > 0.05)
        color_lit = float3(0.35,0.35,0.35) * color_lit;
    else
        color_lit = float3(0.1,0.1,0.1) * color_lit;
