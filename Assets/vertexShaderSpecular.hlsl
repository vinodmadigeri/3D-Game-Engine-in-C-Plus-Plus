...
void main( in const float3 i_position_model : POSITION, in const float2 i_uv : TEXCOORD0, in const float3 i_normal_model : NORMAL, in const float3 i_color : COLOR0,
	out float4 o_position_screen : POSITION, out float3 o_position_world : POSITION1, out float3 o_normal_world : NORMAL, out float2 o_uv : TEXCOORD0, out float3 o_color : COLOR0 )
{
	// Calculate position
	{
		...
		//Out put position world for specular light calculation
